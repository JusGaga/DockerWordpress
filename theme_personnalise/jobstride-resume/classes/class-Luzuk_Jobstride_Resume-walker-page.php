 ...
 *
 * @package WordPress
 * @subpackage Jobstride_Resume
 * @since Jobstride Resume 1.0
 */

if ( ! class_exists( 'Jobstride_Resume_Walker_Page' ) ) {
	/**
	 * CUSTOM PAGE WALKER
	 ...
	 * @since Jobstride Resume 1.0
	 */
	class Jobstride_Resume_Walker_Page extends Walker_Page {

		/**
		 * Outputs the beginning of the current element in the tree.
		 *
		 * @since Jobstride Resume 1.0
		 * @since Jobstride Resume 1.9 Renamed `$page` to `$data_object` and `$current_page` to `$current_object_id`
		...
		public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
			...
			if ( '' === $page->post_title ) {
				...
				$page->post_title = sprintf( __( '#%d (no title)', 'jobstride-resume' ), $page->ID );
			}
